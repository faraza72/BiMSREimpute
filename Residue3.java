...
public class Residue3
{

    static int I, J, K,mn=0,percent=0,missing_no=765;
    double zij, ziJ, zIj, zIJ, Hij = 0, Rij, R, sum=0, dist;
	double msdAdd=0,add=0, mean=0, deviation1=0, add1=0, add2=0, final_deviation=0,min;
    ...
    public static void main(String args[]) 
	{
		...
		if((missing_no) > 0 && (missing_no < 800))
			percent=1;
		else if((missing_no) > 800 && (missing_no < 4000))
			percent=5;
		else if((missing_no) > 4000 && (missing_no < 8000))
			percent=10;
		else if((missing_no) > 8000 && (missing_no < 11000))
			percent=15;
		else
			percent=20;
	...
	void Cal_deviation(double mean)
	{
		...
		//String ="Final_Matrix"+StringValueExppercent+gene_no";
		try
		{
		File f = new File("Final_matrix.txt");
		if (f.isFile()) 
		{
		long size = f.length();
		if (size != 0) 
			...
			FileWriter fwOb = new FileWriter("Final_Matrix.txt", false); 
		...
		try {
        BufferedWriter bw = new BufferedWriter(new FileWriter("Final_matrix.txt"));
		
		bw.write(String.valueOf(percent));
		bw.newLine();
		bw.write(String.valueOf(gene_no));
		bw.newLine();
		...
		//final_dev[d]=final_deviation;
		System.out.println("Final Deviation : "+final_deviation);
		try 
		{
            FileWriter writer = new FileWriter("Final_Deviation(10%).txt", true);
			writer.write(String.valueOf(percent));
            writer.write("\t");
			writer.write(String.valueOf(gene_no));
            writer.write("\t");
            writer.write(String.valueOf(final_deviation));
			writer.write("\r\n"); // write new line
            writer.close();
        } 
		catch (IOException e) 
		...
		/*FileWriter fwOb = new FileWriter("Final_Deviation.txt", false); 
			PrintWriter pwOb = new PrintWriter(fwOb, false);
			pwOb.flush();
			pwOb.close();
			fwOb.close();*/
	...
	void sort(double[] H, int[] H_index, int row)
	{
		...
		/*for(int k=0;k<final_dev.length;k++)
		{
			if(final_dev[k]<min)
				min=final_dev[k];
		}*/
